...
def create_eval_sample(doc, max_span_size: int):
    ...
    # my approach: parse tree for span selection

    # span_index = get_index_span(doc)
    # for (span_x,span_y) in span_index:
    #   # print("Start check")
    #   # for i in range(span_x, span_y+1):
    #   #   print(array[i])
    #   # print("------")
    #   # for x in doc.tokens[span_x:span_y+1]:
    #   #    print(x)
    #   # print("End check")
    #   spanpair = doc.tokens[span_x:span_y+1].span
    #   entity_spans.append(spanpair)
    #   entity_masks.append(create_entity_mask(*spanpair, context_size))
    #   entity_sizes.append(span_y+1-span_x)
    
    # their approach: limit max_span_size (10)
